...
async function prediction(type, typeid) {
    ...
    console.log("Bypassing balance check. Prediction will work even if balance is 0.");

    // 🔹 Forcefully setting balance to a valid number (Even if it's 0.xx)
    userbalance = Math.max(userbalance, 1); // Minimum 1 set kar diya

    if (play_game === false) {
        toolBoxAlert('Your account is not active for getting prediction!', true);
        handle_section(window.location.hash);
        return false;
    }

    console.log("Prediction system activated.");
}
